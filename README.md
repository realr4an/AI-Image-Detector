...
Identify whether a portrait was generated by an AI model or captured in the real world. The project combines face extraction via YOLOv8 with a set of TensorFlow classifiers (ResNet50 and MobileNetV2) and provides Streamlit apps for interactive exploration, evaluation scripts, and training utilities.
...
- Self-contained workflow: generated assets (`Data/`, `Models/`, figures) are recreated when needed via the tooling.
...
> Directories such as `Data/`, `Models/`, or `docs/figures/` are generated by the scripts. Run the quickstart commands to recreate them locally.
```
